...
int otaNO = 6;//ลำดับของบอร์ดที่จะทำการอัพโหลด([แปลงที่ 1 ใช้เลข 1])
...
const char *topic_soil1 = ("soil"+String(TGS-1)).c_str();//ชื่อของ topic ที่ต้องการส่งขึ้น broker ของ 15 cm
const char *topic_soil2 = ("soil"+String(TGS)).c_str();//ชื่อของ topic ที่ต้องการส่งขึ้น broker ของ 30 cm
...
void reconnect() {
  if (!client.connected()) {//เงื่อนไขเมื่อเชื่อมต่อกับ mqtt ไม่ได้
    ...
    if (client.connect(("npru_ee_soil_moisture"+String(otaNO)).c_str())) {//ใส่ข้อความอะไรก็ได้ที่ท่านคิดว่าจะไม่ซ้ำกับผู้อื่นหรือไม่ซ้ำกันในจำนวนมาก(หากไม่สามารถเชื่อมต่อกับ mqtt ได้ลองทำการเปลี่ยนข้อความ)
