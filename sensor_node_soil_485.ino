...
int otaNO = 5;//ลำดับของบอร์ดที่จะทำการอัพโหลด([แปลงที่ 1 ใช้เลข 1])
const char *topic_soil1 = "soil9";//ชื่อของ topic ที่ต้องการส่งขึ้น broker ของ 15 cm
...
#define WiFi_PASSWORD "47845862" //password หรือรหัสในการเชื่อมต่อ WiFi
...
void reconnect() {
  if (!client.connected()) {//เงื่อนไขเมื่อเชื่อมต่อกับ mqtt ไม่ได้
    ...
    if (client.connect("mqtt_mois_RS485_5")) {//ใส่ข้อความอะไรก็ได้ที่ท่านคิดว่าจะไม่ซ้ำกับผู้อื่นหรือไม่ซ้ำกันในจำนวนมาก(หากไม่สามารถเชื่อมต่อกับ mqtt ได้ลองทำการเปลี่ยนข้อความ)
